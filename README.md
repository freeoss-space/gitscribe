...
1. Running `gitscribe` via Node.js `child_process.spawnSync` to capture real
   ANSI output.
2. Serving an HTML page that embeds [xterm.js](https://xtermjs.org) and
   writes the captured output to the terminal widget.
3. Using [Playwright](https://playwright.dev) to drive a headless Chromium
   window, wait for the terminal to finish rendering, and clip-screenshot the
   terminal `<div>`.
...
const out = spawnSync('./gitscribe', ['commit', '--no-interface'],
                      { encoding: 'utf8' }).stdout;

// … serve HTML page with xterm.js, write `out` to terminal, screenshot …
```

This produces real PNG files from a real browser with proper font rendering,
ANSI colour, and the Catppuccin Mocha colour scheme — no SVG hand-crafting.
...
All screenshots below are real PNGs captured by Playwright driving a headless
Chromium window that renders an [xterm.js](https://xtermjs.org) terminal.
The actual gitscribe binary output was piped into xterm.js with ANSI colour
sequences intact before each screenshot was taken.

### Build and run

![build and run](docs/screenshots/01-build-and-run.png)
...
![commit no-interface](docs/screenshots/03-commit-no-interface.png)
...
![config json](docs/screenshots/05-config-json.png)
