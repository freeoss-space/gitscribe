...
// Model is the Bubble Tea model for the PR TUI.
type Model struct {
	uc         *prUC.UseCase
	ctx        context.Context
...
// New creates the model and starts generation.
func New(ctx context.Context, uc *prUC.UseCase, baseBranch string) Model {
...
func (m Model) Init() tea.Cmd {
	return tea.Batch(m.spinner.Tick, generateCmd(m.ctx, m.uc, m.baseBranch, ""))
}
...
func (m Model) handleKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case "ctrl+r":
		if m.state == stateEditing || m.state == stateError {
			...
			return m, tea.Batch(m.spinner.Tick, generateCmd(m.ctx, m.uc, m.baseBranch, ""))
		}
...
func (m Model) submitFeedback() (tea.Model, tea.Cmd) {
	...
	m.title.SetValue("")
	m.title.Placeholder = "PR title…"
	m.state = stateGenerating
	return m, tea.Batch(m.spinner.Tick, generateCmd(m.ctx, m.uc, m.baseBranch, feedback))
}

func (m Model) doCreatePR() (tea.Model, tea.Cmd) {
	return m, createPRCmd(m.uc, m.currentMessage())
}
...
func (m Model) View() tea.View {
	...
	sb.WriteString(styles.Title.Render("gitscribe pr") + "\n\n")

	switch m.state {
	case stateGenerating:
		sb.WriteString(styles.Muted.Render(m.spinner.View() + " Generating PR description…"))

	case stateEditing:
		sb.WriteString(styles.Muted.Render("Title") + "\n")
		sb.WriteString(m.title.View() + "\n\n")
		sb.WriteString(styles.Muted.Render("Description") + "\n")
	...
	case stateDone:
		sb.WriteString(styles.Success.Render("✓ PR created!"))
	}
...
func helpText() string {
	return styles.Help.Render(
		"enter/ctrl+s: create PR • tab: switch field • ctrl+r: regenerate • ctrl+f: feedback • ctrl+c: copy • esc: exit",
	)
}

func generateCmd(ctx context.Context, uc *prUC.UseCase, baseBranch, feedback string) tea.Cmd {
	return func() tea.Msg {
		msg, err := uc.Generate(ctx, baseBranch, feedback)
		if err != nil {
...
func createPRCmd(uc *prUC.UseCase, msg domain.Message) tea.Cmd {
	return func() tea.Msg {
		if err := uc.CreateGitHubPR(msg); err != nil {
			return prErrMsg{err: fmt.Errorf("create PR: %w", err)}
		}
		return prCreatedMsg{}
	}
}
