...
All features were validated using two complementary headless approaches.

### 1 — `script(1)` session recorder

`script` is a POSIX utility that captures stdin/stdout/stderr from a
subcommand without requiring a graphical display:
...
### 2 — Playwright + xterm.js (screenshot capture)

The documentation screenshots were generated by:

1. Spawning a real `bash` shell in a **node-pty** PTY (cols × rows configured
   per screenshot).
2. Writing the gitscribe command to the shell's stdin — `bash` executes the
   real binary, which writes its real output to the PTY.
3. Collecting every byte the PTY produces (ANSI escape sequences included).
4. Serving an HTML page with [xterm.js](https://xtermjs.org) that calls
   `term.write(bytes)` with those verbatim bytes.
5. Using [Playwright](https://playwright.dev) to drive a headless Chromium
   window, wait for xterm.js to finish rendering, and clip-screenshot the
   terminal `<div>`.
...
const proc = pty.spawn('/bin/bash', ['--norc', '--noprofile'], {
  name: 'xterm-256color', cols: 60, rows: 10,
  env: { ...process.env, PS1: '\\[\\e[1;32m\\]$ \\[\\e[0m\\]',
         PATH: `/tmp:${process.env.PATH}` },
});
proc.onData(chunk => chunks.push(Buffer.from(chunk, 'binary')));
proc.write('gitscribe commit --no-interface\n');
// … wait for second prompt, then feed chunks verbatim into xterm.js …
```

The bytes that reach xterm.js are exactly what a real terminal emulator would
receive — nothing is mocked or interpolated.

For VT100/VT220 screen-state assertions in Python tests, the
[`pyte`](https://github.com/selectel/pyte) library provides a headless
terminal emulator:
...
Each screenshot is a real PNG taken by Playwright from a headless Chromium
window running [xterm.js](https://xtermjs.org).  The binary is spawned inside
a real `bash` shell inside a real PTY via **node-pty** — every byte rendered
by xterm.js came verbatim from the running process, with no strings mocked or
injected.

### `gitscribe commit --no-interface`

![commit no-interface](docs/screenshots/01-build-and-run.png)

### `gitscribe commit` — interactive TUI

![commit TUI](docs/screenshots/02-commit-tui.png)

### `gitscribe commit --no-interface` (close-up)

![commit no-interface close-up](docs/screenshots/03-commit-no-interface.png)

### `gitscribe pr --no-interface`

![pr command](docs/screenshots/04-pr-command.png)

### Unknown command error handling

![unknown command](docs/screenshots/05-unknown-command.png)

### Test suite — all passing

![test suite](docs/screenshots/06-test-suite.png)
...
|---|---|---|
| [xterm.js](https://github.com/xtermjs/xterm.js) | MIT | Full VT100/xterm terminal emulator in the browser; renders the ANSI output with accurate colour and font |
| [Playwright](https://github.com/microsoft/playwright) | Apache 2.0 | Headless Chromium automation used to load the xterm.js page and clip-screenshot the terminal widget into PNGs |
